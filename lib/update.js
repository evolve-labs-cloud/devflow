...
async function updateCommand(targetArg, options) {
  ...
  // Resolve source: --source flag overrides PACKAGE_ROOT (useful in local dev)
  const sourceRoot = options.source
    ? path.resolve(options.source)
    : PACKAGE_ROOT;

  if (options.source) {
    info(`Source: ${sourceRoot} (--source override)`);
  }
  ...
  if (installedVersion === VERSION && !options.force) {
    ...
    info('Use --force to re-copy all files even when version matches.');
    return;
  }
  ...
  if (!options.force) {
    ...
    info('Force mode: re-copying all files (version already matches).');
    console.log();
  }
  ...
  await copyDir(
    path.join(sourceRoot, '.claude', 'commands', 'agents'),
    path.join(targetDir, '.claude', 'commands', 'agents')
  );
  ...
  await copyDir(
    path.join(sourceRoot, '.claude', 'commands', 'quick'),
  ...
  for (const file of ['devflow-help.md', 'devflow-status.md']) {
    try {
      await copyFile(
        path.join(sourceRoot, '.claude', 'commands', file),
  ...
  await copyFile(
    path.join(sourceRoot, '.claude_project'),
  ...
  await copyDir(
    path.join(sourceRoot, '.devflow', 'agents'),
  ...
  try {
    ...
    await copyFile(
      path.join(sourceRoot, '.devflow', 'project.yaml'),
      projectYamlPath
    );
  ...
  try {
    ...
    if (!claudeSettings.mode) {
      claudeSettings.mode = 'plan';
    }
    ...
    success('Agent Teams + plan mode enabled (.claude/settings.json)');
  } catch (err) {
