...
class MemoryManager {
  ...
  getContextForAgent(agent, limit = 5) {
    const recentDecisions = this.getRecentDecisions(limit);
    ...
    if (recentDecisions.length > 0) {
      context += `### Recent Decisions (last ${limit}):\n`;
      recentDecisions.forEach(d => {
        ...
        if (d.alternatives && d.alternatives.length > 0) {
          context += `  Alternatives: ${d.alternatives.join(', ')}\n`;
        }
      });
      context += '\n';
    }

    // Recent artifacts by this agent
    if (recentArtifacts.length > 0) {
      context += `### Recent Artifacts by @${agent}:\n`;
