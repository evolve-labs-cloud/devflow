...
4. Avaliar confiança no review (0-100%)
 ...
 */
function buildPrompt(agent, specContent, previousOutputs, agentDefinition, memoryContext) {
 ...
 * Extracts confidence score, verdict, critical gaps and challenged points.
 *
 * @param {string} output - Raw Challenger output
 * @returns {{ confidence: number|null, verdict: string|null, criticalGaps: string[], challengedPoints: number }}
 */
function parseChallengerOutput(output) {
  ...
  return { confidence, verdict, criticalGaps, challengedPoints };
}
