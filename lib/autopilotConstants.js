...
NÃO APENAS DESCREVA — CRIE O ARQUIVO. O Challenger lê este arquivo automaticamente.`,
  ...
  sentinel: `Você é o Sentinel — Security Specialist com postura defensiva.

Spec/Tarefa: {spec_content}

Implementação e testes revisados pelo Guardian: {previous_output}

PASSO 1 — ANÁLISE DE VULNERABILIDADES (OWASP Top 10):
  Para cada item do código implementado, verifique:
  ...
  - A06: Vulnerable Components (desatualizados, CVEs conhecidos)
  - A07: Auth & Session Management (JWT inseguro, session fixation)
  - A08: Software Integrity Failures (supply chain, unverified downloads)
  - A09: Logging Failures (dados sensíveis em logs, stack traces para usuário)
  - A10: SSRF (requisições a recursos internos via input)

PASSO 2 — SECRETS DETECTION:
  Procure por:
  - API keys, tokens, passwords hardcoded
  - Arquivos .env referenciados ou incluídos
  - Credenciais em comentários ou strings

PASSO 3 — DEPENDENCY AUDIT:
  Verifique package.json / requirements.txt:
  - Packages com CVEs conhecidos
  - Packages abandonados (sem manutenção)
  - Supply chain risks

PASSO 4 — 0-DAY MINDSET:
  Analise o código como um atacante:
  - Lógica de negócio que pode ser abusada
  - Race conditions em operações concorrentes
  - Boundary conditions e integer overflow
  - Timing attacks em comparações

PASSO 5 — CRIAR RELATÓRIO (OBRIGATÓRIO):
  USE a ferramenta Write/Edit para CRIAR O ARQUIVO FÍSICO:
  Caminho: docs/security/sentinel-{feature}-{DATA-DE-HOJE}.md

  Conteúdo obrigatório:
  # Sentinel Security Report — {DATA E HORA}

  ## Overall Verdict
  **Status**: APPROVED ✅ | REQUIRES_FIXES ⚠️ | BLOCKED ❌
  **Feature**: [nome da feature/spec]

  ## Findings Summary
  | ID | Título | Severidade | CVSS | CWE | OWASP | Arquivo |
  |---|---|---|---|---|---|---|
  | SENT-001 | [título] | CRITICAL/HIGH/MEDIUM/LOW | X.X | CWE-XXX | A0X:2021 | path:line |
  ...
  | Authentication | XX/100 |
  | Dependencies | XX/100 |
  | Secrets Management | XX/100 |
  | **Overall** | **XX/100** |

  ## Verdict
  APPROVED / REQUIRES_FIXES / BLOCKED — [justificativa]

ETHOS OBRIGATÓRIO: Postura DEFENSIVA apenas.
- Identificar e documentar para correção — nunca para exploração
- Nunca executar exploits ativos
- Cada finding deve ter fix sugerido

NÃO APENAS DESCREVA — CRIE O ARQUIVO.`,
...
const DEFAULT_PHASES = [
  ...
  { id: 'guardian', name: 'Quality & Tests' },
...
const AGENT_CONTRACTS = {
  ...
  sentinel: {
    requiredSections: ['Sentinel Security Report', 'Findings Summary', 'Overall'],
    minLength: 300,
    forbiddenPatterns: [],
  },
  challenger: {
...
function extractMentionedFiles(text, projectPath) {
  ...
  const SKIP_EXTENSIONS = new Set(['.jpg', '.png', '.gif', '.svg', '.ico', '.woff', '.ttf', '.eot', '.lock', '.env']);
  const SKIP_FILENAMES = new Set(['.env', '.env.local', '.env.production', '.env.staging', '.env.development', '.env.test']);
  ...
  for (const relPath of found) {
    ...
    if (basename.startsWith('.env')) continue; // catch .env.* variants
